<template>
  <div class="w-auto max-w-[800px] ml-20" @click="closeAddTodo">
    <div class="space-y-5">
      <div v-for="category in categoryStore.categories" :key="category.id">
        <div
          ...
          <div class="flex items-center gap-2 flex-1 overflow-hidden">
            ...
            <span class="text-sm font-bold truncate" :style="{ color: category.color }">{{ truncateText(category.title) }}</span>
          </div>
          <span class="text-gray-400 ml-2 flex-shrink-0">+</span>
        </div>
        <div v-if="selectedCategory && selectedCategory.id === category.id" class="mt-4 space-y-2" @click.stop>
